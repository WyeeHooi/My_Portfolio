              ...
              <ul className="w-full h-[10rem] md:h-[25rem] sm:h-[10rem]">
                {collabApps.map((app, index) => {
                  ...
                  return (
                    <li
                      key={app.id}
                      className={`absolute top-0 left-1/2 h-1/2 -ml-[0.3rem] md:-ml-[1.6rem] origin-bottom`}
                      style={{ transform: `rotate(${angle}deg)` }} // Apply the calculated angle
                    >
                      <div
                        className={`relative -top-[0.3rem] md:-top-[1.6rem] flex w-[1.5rem] h-[2rem] md:w-[4rem] md:h-[4rem] bg-n-7 border group border-n-1/15 rounded-xl`}
