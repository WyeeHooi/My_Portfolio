        ...
        <div className="relative">
          <div className="relative z-1 flex items-center h-[39rem] mb-5 p-8 border border-n-1/30 rounded-3xl overflow-hidden lg:p-20 xl:h-[46rem]">
            ...
            <div className="relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
              ...
              <ul className="relative w-[20rem] h-[20rem]">
                {collabApps.map((app, index) => (
                  <li
                    key={app.id}
                    className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${(index * 360) / collabApps.length}`}
                  >
                    <div className="relative -top-[1.6rem] flex w-[5rem] h-[5rem] bg-n-7 border group border-n-1/15 rounded-xl -rotate-${(index * 360) / collabApps.length}">
                      <img
