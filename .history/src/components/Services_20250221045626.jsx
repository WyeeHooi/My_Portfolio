            ...
            <div className="relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
              ...
              <ul className="relative w-[30rem] h-[30rem]">
                {collabApps.map((app, index) => (
                  <li
                    key={app.id}
                    className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${index * 30
                      }`}
                  >
                    <div
                      className={`relative -top-[1.6rem] flex w-[5rem] h-[5rem] bg-n-7 border group border-n-1/15 rounded-xl -rotate-${index * 30
                        }`}
                    >
                      <img
