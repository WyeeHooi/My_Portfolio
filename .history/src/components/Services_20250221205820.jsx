    ...
    <Section id="how-to-use">
      <div className="container">
        ...
        <div className="relative">
          <div className="relative z-1 flex items-center h-[30rem] mb-5 p-8 border border-n-1/30 rounded-3xl overflow-hidden lg:p-20 xl:h-[46rem] lg:h-[50rem] md:h-[40rem] sm:h-[30rem]">
            ...
            <div className="absolute z-10 p-5 top-[10%] left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
              ...
              <ul className="w-full h-[10rem] md:h-[25rem] sm:h-[10rem]">
                {collabApps.map((app, index) => {
                  ...
                  return (
                    <li
                      key={app.id}
                      className={`absolute top-0 left-1/2 h-1/2 -ml-[1rem] md:-ml-[1.6rem] origin-bottom z-5 `}
                      style={{ transform: `rotate(${angle}deg)` }} // Apply the calculated angle
                    >
                      <div
                        className={`z-0 relative -top-[0.3rem] md:-top-[1.6rem] flex w-[3rem] h-[3rem] md:w-[4rem] md:h-[4rem] bg-n-7 border group border-n-1/15 rounded-xl `}
                        style={{ transform: `rotate(-${angle}deg)` }} // Counter-rotate the content
                      >
                        <img
                          className="m-auto w-full h-auto max-w-[3rem] sm:max-w-[4rem] transition-transform duration-300 transform scale-80 hover:scale-110 sm:scale-100"
                          alt={app.title}
                          src={app.icon}
                        />
